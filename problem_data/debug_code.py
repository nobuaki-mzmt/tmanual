...
problem_pickle = "tmanual/res.pickle"
...
def postanalysis(in_dir, out_dir, scale_object_len, contact_threshold, network_out, output_image, object_size, font_size, text_drawing):
    ...
    # Data read
    with open(problem_pickle, mode='rb') as f:
        tmanual_output = pickle.load(f)
        ...
        with open(problem_pickle, mode='wb') as f:
    ...
    for i_df in tqdm(range(len(tmanual_output[0]))):
        ...
        if len_t > 0:
            ...
            #for tt_n in range(237):
                min_dis = 99999
                nearest_point = np.array([0,0])
                for tt_t in range(len_t):
                    if tt_n != tt_t:
                        if norm(node[0][tt_n]-node[0][tt_t]) < contact_threshold: # if two tunnels start from the same point
                            continue
                        ll = len(tunnel[tt_t])
            ...
            while True:
                ...
                if len(check_tunnel) == 0:
                    if min(tunnel_sequence) < 0:
                        print("Unexpected error in " + img_data.name + ": cannot get tunnel id.")
